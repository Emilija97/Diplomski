...
import { Activity, ActivityName, ActivityStatus, activityStatusMap } from "../store/activities-state";
...
function Test(props: Props) {
  ...
  statusMap.set("passed", "Passed");
  statusMap.set("notPassed", "Not passed");
  const [selectedStatus, setSelectedStatus] = useState(props.id === undefined ? "" : activityStatusMap.get(props.status as number) as string);
  ...
  const onAcceptClick = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    const activity: Activity = {
      ...
      status: (selectedStatus === "" ? ActivityStatus.PENDING : (selectedStatus === "passed" ? ActivityStatus.PASSED : ActivityStatus.NOT_PASSED)),
      comment: comment
    }
  ...
  return (
    <div className="test activity-content">
      <DatePickerComponent
        ...
        title="Select date"
      />
      <label className="test__label">Select status:</label>

      <ButtonToggle
        buttonToggleMap={statusMap}
        initState={selectedStatus}
